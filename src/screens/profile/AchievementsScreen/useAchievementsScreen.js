...
function useAchievementsScreen() {
  ...
  var fetchAchievements = useCallback(function () {
    ...
    // Fetch both gamification and achievements data
    ...
    Promise.all([achievementsPromise, gamificationPromise])
      .then(function (results) {
        ...
        var merged = DEFAULT_ACHIEVEMENTS.map(function (def) {
          ...
          if (match) {
            return {
              ...
              progress: match.progress || match.currentProgress || 0,
              unlocked: !!(match.unlocked || match.earned || match.completed),
        ...
        serverAchievements.forEach(function (s) {
          ...
          if (!exists) {
            merged.push({
              ...
              progress: s.progress || s.currentProgress || 0,
              unlocked: !!(s.unlocked || s.earned || s.completed),
        ...
        if (gamData) {
          setTotalXp(gamData.totalXp || gamData.xp || 0);
          setLevel(gamData.level || 1);
          setXpToNext(gamData.xpToNext || gamData.xpToNextLevel || 100);
      ...
      .catch(function (err) {
        ...
        var defaultMerged = DEFAULT_ACHIEVEMENTS.map(function (def) {
          return Object.assign({}, def, { progress: 0, unlocked: false, unlockedAt: null });
        });
